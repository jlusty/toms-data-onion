...
fn main() {
    ...
    println!("KEK: {:?}", kek);
    println!("Key IV: {:?}", key_iv);
    println!("Key: {:?}", key);

    let decrypted_key: &mut [u8] = &mut [];
    let aes_kek = &AesKey::new_decrypt(kek).expect("Unable to create AesKey");
    // Decrypt KEK
    let bytes_in_key = unwrap_key(aes_kek, Some(key_iv), decrypted_key, key).unwrap();
    println!("Bytes in key: {}", bytes_in_key);
